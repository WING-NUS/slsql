...
def evaluate(gold, predict, db_dir, kmaps):
    ...
    cnt = 0
    scores = 0
    for id, (p, g) in enumerate(zip(predict, glist), 1):
        cnt += 1
        p = ' '.join(p)
        scores += do_score(evaluator, db_dir, kmaps, p, g)
    return scores / cnt
...
def evaluate_dev(dev_pred):
    table = 'data/tables.json'
    kmaps = build_foreign_key_map_from_json(table)

    return evaluate(gold='data/dev_gold.sql', predict=dev_pred, db_dir='data/database', kmaps=kmaps)
